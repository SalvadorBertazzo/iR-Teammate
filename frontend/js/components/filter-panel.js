...
export function renderFilterPanel(currentFilters = {}) {
    ...
    return `
        <div class="bg-white rounded-lg p-4 border border-gray-200 shadow-sm h-full flex flex-col overflow-hidden">
            <div class="flex items-center justify-between mb-3 flex-shrink-0">
                <h3 class="text-lg font-semibold text-gray-900">Filters</h3>
                <button id="clear-filters" class="text-sm text-blue-600 hover:text-blue-700">
                    Clear all
                </button>
            </div>

            <form id="filter-form" class="flex-1 flex flex-col gap-3 min-h-0 overflow-hidden">
                <!-- Category -->
                    ...
                    <div class="space-y-1">
                        ${categories.map(cat => `
                            <label class="flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-gray-700 text-xs">${cat.label}</span>
                            </label>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 flex-shrink-0">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Min iRating</label>
                        <input type="number" name="min_irating" value="${currentFilters.min_irating || ''}"
                            class="w-full bg-white border border-gray-300 rounded-md px-2 py-1.5 text-sm text-gray-900 placeholder-gray-400"
                            placeholder="0" min="0" step="100">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Max iRating</label>
                ...
                <!-- License Level -->
                <div class="flex-shrink-0">
                    <label class="block text-xs font-medium text-gray-700 mb-1">Min License</label>
                    <select name="min_license_level"
                        class="w-full bg-white border border-gray-300 rounded-md px-2 py-1.5 text-sm text-gray-900">
                ...
                <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-1 flex-shrink-0">Series</label>
                    <div class="space-y-1 flex-1 overflow-y-auto border border-gray-200 rounded-md p-2 bg-gray-50">
                        ${catalogs.series.map(series => `
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="series_ids" value="${series.id}"
                                    ...
                                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                <span class="text-gray-700 text-xs">${escapeHtml(series.name)}</span>
                            </label>
                        `).join('')}
                    </div>
                </div>
                ...
                <!-- Cars -->
                <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-1 flex-shrink-0">Car</label>
                    <div class="space-y-1 flex-1 overflow-y-auto border border-gray-200 rounded-md p-2 bg-gray-50">
                        ${catalogs.cars.map(car => `
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="car_ids" value="${car.id}"
                                    ...
                                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                <span class="text-gray-700 text-xs">${escapeHtml(car.name)}</span>
                            </label>
                        `).join('')}
                    </div>
                </div>
                ...
                <!-- Track -->
                <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
                    <label class="block text-sm font-medium text-gray-700 mb-1 flex-shrink-0">Track</label>
                    <div class="space-y-1 flex-1 overflow-y-auto border border-gray-200 rounded-md p-2 bg-gray-50">
                        ${catalogs.tracks.map(track => `
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="track_ids" value="${track.id}"
                                    ...
                                    class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                <span class="text-gray-700 text-xs">${escapeHtml(track.name)}</span>
                            </label>
                        `).join('')}
                    </div>
                </div>
                ...
                <button type="submit"
                    class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-1.5 px-4 rounded-md transition-colors text-sm flex-shrink-0">
                    Apply Filters
                </button>
            </form>
        </div>
