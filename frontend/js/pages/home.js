        ...
        <div class="flex flex-col lg:flex-row gap-6">
            <main class="flex-1 order-2 lg:order-1">
                <div class="flex items-center justify-between mb-4">
                    <h1 class="text-2xl font-bold text-content-primary">Find Teammates</h1>
                    <div id="results-count" class="text-content-secondary"></div>
                </div>
                <!-- Search bar -->
                <div class="mb-6">
                    <div class="relative">
                        <input type="text" id="search-input"
                            class="w-full form-input rounded-lg px-4 py-3 pl-10"
                            placeholder="Search posts...">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-content-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    function renderPagination(response) {
        ...
        pagination.innerHTML = `
            <div class="flex items-center justify-center gap-2">
                <button class="pagination-btn px-3 py-1.5 rounded btn-secondary disabled:opacity-50"
                    data-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''}>
                    Previous
                </button>
                ${pages.map(page => {
                    if (page === '...') {
                        return '<span class="px-2 text-content-muted">...</span>';
                    }
                    const isActive = page === currentPage;
                    return `
                        <button class="pagination-btn px-3 py-1.5 rounded ${isActive ? 'btn-primary' : 'btn-secondary'}"
